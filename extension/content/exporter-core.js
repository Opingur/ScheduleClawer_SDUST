(() => {
  ...
  const hasCourseMeta = (value) => /(?:老师|教师|任课教师)\s*[:：]/.test(value) && (/(?:时间|上课时间)\s*[:：]/.test(value) || /\d{1,2}\s*(?:-|~|—|–|至)\s*\d{1,2}\s*周/.test(value));
  ...
  const ADAPTERS = [
    {
      id: 'generic-course-card-v1',
      canHandle: () => true,
      extract: readGenericSchedule
    }
  ];

  function readSchedule() {
    const adapter = ADAPTERS.find((candidate) => candidate.canHandle());
    if (!adapter) throw new Error('当前页面没有匹配的课表适配器。');
    return { ...adapter.extract(), adapterId: adapter.id };
  }
  ...
  function field(text, names) {
    ...
    const boundaries = '老师|教师|任课教师|时间|上课时间|地点|教室|上课地点|课程编号|班级|总人数|考核方式|总学时|网课群号|网课链接|分组名';
  ...
  function courseName(text) {
    const marker = String(text || '').search(/(?:老师|教师|任课教师|时间|上课时间|地点|教室|上课地点)\s*[:：]/);
  ...
  function inferredScheduleText(text) {
    const match = String(text || '').match(/\d{1,2}\s*(?:-|~|—|–|至)\s*\d{1,2}\s*周(?:\s*(?:单周|双周|\(单\)|\(双\)|（单）|（双）))?\s*(?:\[|【|\(|（)\s*\d{1,2}\s*(?:-|~|—|–|至)\s*\d{1,2}\s*节?\s*(?:\]|】|\)|）)/);
  ...
  function normalise(cards, timeMap, maxWeek) {
    ...
    for (const card of cards) {
      ...
      const inferredTime = inferredScheduleText(text);
      const sourceTime = field(text, ['上课时间', '时间']) || inferredTime;
      const teacher = clean(field(text, ['任课教师', '老师', '教师']).replace(inferredTime, ''));
      const location = field(text, ['上课地点', '地点', '教室']);
  ...
  function loadLibrary() {
    ...
    return Promise.reject(new Error('插件内置 Excel 组件未加载，请在扩展管理页重新载入插件后再试。'));
  }
  async function run(rawConfig) {
    ...
    if (config.weekCount < 1 || config.weekCount > 30) throw new Error('学期总周数应为 1–30。');
    const monday = new Date(`${config.termStartMonday}T00:00:00`);
    if (Number.isNaN(monday.getTime()) || monday.getDay() !== 1) throw new Error('第 1 周周一无效，请在插件中填写校历上的周一日期。');

    toast('正在读取当前课表…');
    const raw = readSchedule();
    const courses = normalise(raw.cards, raw.timeMap, config.weekCount);
    if (!courses.length) throw new Error('没有识别到课程。请确认已打开课表、周次为“全部”并已点击查询。');
    toast(`已读取 ${courses.length} 条课程记录，正在生成 Excel…`);
    await loadLibrary();
    const book = buildWorkbook(courses, raw.timeMap, config);
    const filename = `课表_${config.termStartMonday}.xlsx`;
    XLSX.writeFile(book, filename, { compression: true });
    toast(`导出完成：${courses.length} 条课程记录。`);
    return { courseCount: courses.length, filename, adapterId: raw.adapterId };
  }

  window.SDUSTScheduleExporter = { run };
})();
